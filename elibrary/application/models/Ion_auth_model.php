...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->db = $this->load->database($this->config->item('connection', 'ion_auth'), TRUE);
	...
	public function hash_check($user, $password)
	{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->db->insert($this->tables['profiles'], $this->_filter_data($this->tables['profiles'], $additional_data));
	...
	public function user($id = NULL)
	{
		...
		$this->select(
			$this->tables['users'].'.*,
			'.$this->tables['profiles'].'.first_name,
			'.$this->tables['profiles'].'.last_name,
			'.$this->tables['profiles'].'.gender,
			'.$this->tables['profiles'].'.tempat_lahir,
			'.$this->tables['profiles'].'.tanggal_lahir,
			'.$this->tables['profiles'].'.address,
			'.$this->tables['profiles'].'.desa_id,
			'.$this->tables['profiles'].'.avatar,
			'.$this->tables['profiles'].'.id as profile_id
		');
		$this->db->join('profile', $this->tables['users'].'.id = '.$this->tables['profiles'].'.user_id', 'left');
