...
class Template
{
	...
	public function add_stylesheet($source)
	{
		$url = asset($source);
		$stylesheet	= '<link href="'.$url.'" rel="stylesheet">';

		$this->_stylesheet .= $stylesheet;

		$this->inject_partial('stylesheet', $this->_stylesheet);
	}

	public function add_script($source)
	{
		$url = asset($source);
		$script	= '<script src="'.$url.'"></script>' . PHP_EOL;

		$this->_scripts .= $script;

		$this->inject_partial('script', $this->_scripts);
	}
}
